...
def overlay_segmentation(image_volume: np.ndarray, mask_volume: np.ndarray, slice_index: Optional[int] = None, opacity: float = 0.45) -> go.Figure:
    ...
    - slice: axial slice index (defaults to center or the slice containing the lesion)
    - colormap: 1 cyan, 2 yellow, 3 red

    Behavior changes: if the center slice contains no segmentation, select the slice with the
    largest segmented area. If the whole volume has no segmentation, return a figure with
    ...
    # Prefer a slice that actually contains segmentation. If slice_index is provided use it
    # (bounded), otherwise attempt to pick the slice with the largest segmented area.
    if slice_index is None:
        # find slices with any mask content
        nonzero_slices = np.any(mask_volume != 0, axis=(1, 2))
        nz_idx = np.where(nonzero_slices)[0]
        if nz_idx.size > 0:
            # pick slice with largest lesion area
            counts = [int(np.count_nonzero(mask_volume[i])) for i in nz_idx]
            slice_index = int(nz_idx[int(np.argmax(counts))])
        else:
            # No segmentation anywhere in volume => return informative figure
            fig = go.Figure()
            fig.update_layout(
                title="Segmentation Overlay (Axial slice) - No segmentation detected",
            ...
            fig.add_annotation(
                text="No segmentation present in the volume",
                xref="paper",
                yref="paper",
                x=0.5,
                y=0.5,
    ...
    fig.update_layout(
        title=f"Segmentation Overlay (Axial slice #{slice_index})",
        template="plotly_dark",
        width=600,
        height=600,
        margin=dict(l=10, r=10, t=40, b=10),
